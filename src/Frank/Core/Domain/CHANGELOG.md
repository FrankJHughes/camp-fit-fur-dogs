...
Frank as a standalone, reusable library.
...
### Changed
- All CQRS abstractions relocated from `CampFitFurDogs.Application` into `Frank`
- DI auto-registration conventions relocated from `CampFitFurDogs.Infrastructure` into `Frank`
- Camp Fit Fur Dogs now references `Frank` for all CQRS and DI contracts
...
The Frank project was created with foundational DDD building blocks.
...
**Tests**
- `Frank.Tests` project scaffold with unit tests for base types
...
| Story | What was created | Later became |
|-------|-----------------|-------------|
| US-014 | .NET Solution Skeleton | `Frank.csproj` project file and test project scaffold |
