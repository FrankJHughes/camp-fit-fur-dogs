...
- commands with and without return values behave correctly  
...
- All validators for a command are executed  
- Validation failures throw `ValidationException`  
- Handlers are resolved deterministically from DI  
- Exactly one handler exists per command  
- Commands with return values return the correct type  
- Commands without return values complete successfully  
- Exceptions thrown by handlers propagate correctly  
- No dynamic dispatch errors occur  
...
- All validators for a command are discovered  
...
- Command with no validators → handler executes  
- Command with one validator → validator runs  
- Command with multiple validators → all run  
...
- Command with handler → executes  
- Command without handler → throws  
- Command with mismatched handler signature → throws  
...
- Handler returning null for TResponse → test should fail  
...
public sealed class TrackingHandler : ICommandHandler<TestCommand>
{
    ...
    public Task HandleAsync(TestCommand command, CancellationToken ct)
    {
        Called = true;
...
- Tests that allow multiple handlers for the same command  

---

# 6. Summary

Testers ensure that the Command Dispatcher:
